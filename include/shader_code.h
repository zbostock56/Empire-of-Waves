...
const char *fragment_shader_wave =
"#version 430 core\n"
...
const char *fragment_shader_rain =
"#version 430 core\n"
"#define C_WIDTH " TO_STRING(C_WIDTH) "\n"
"#define T_WIDTH (8)\n"
"#define PI " TO_STRING(PI) "\n"
...
"#define RANGE (5.0)\n"
""
"#define PUDDLE_DUR " TO_STRING(PUDDLE_DUR) "\n"
"#define PUDDLE_PERIOD " TO_STRING(PUDDLE_PERIOD) "\n"
"#define PUDDLE_FREQ " TO_STRING(PUDDLE_FREQ) "\n"
"#define PUDDLE_THRESH " TO_STRING(PUDDLE_THRESH) "\n"
"#define I " TO_STRING(I) "\n"
"#define J " TO_STRING(J) "\n"
"#define L " TO_STRING(L) "\n"
"#define M " TO_STRING(M) "\n"
"#define N " TO_STRING(N) "\n"
"#define O " TO_STRING(O) "\n"
"#define P " TO_STRING(P) "\n"
""
"out vec4 FragColor;\n"
"in vec2 tex_coords;\n"
"uniform vec2 chunk_coords;\n"
"uniform float time;\n"
"uniform float puddle_offset;\n"
""
"float noise2(vec2);\n"
""
"void main() {\n"
"  vec2 pixel_coords = {\n"
"    floor(C_WIDTH * T_WIDTH * tex_coords.x),\n"
"    floor(C_WIDTH * T_WIDTH * (1.0 - tex_coords.y))\n"
"  };\n"
"  float circles = (sin((2.0 * PI *\n"
"                       (pixel_coords.x - (0.25 * PUDDLE_PERIOD))) /\n"
"                       PUDDLE_PERIOD) +\n"
"                   sin((2.0 * PI *\n"
"                       (pixel_coords.y - (0.25 * PUDDLE_PERIOD))) /\n"
"                       PUDDLE_PERIOD)) / 2.0;\n"
"  vec2 circle_coord = floor(pixel_coords / PUDDLE_PERIOD) +\n"
"                      vec2(puddle_offset);\n"
"  float noise_res = (0.5 * sin(noise2((circle_coord +\n"
"                     (circle_coord / T_WIDTH)) / 8.0))) + 0.5;\n"
"  float dist = (PUDDLE_FREQ * noise_res) + max(circles, 0.0);\n"
"  float delta = ((time + (2.0 * noise_res)) / PUDDLE_DUR) -\n"
"                floor((time + (2.0 * noise_res)) / PUDDLE_DUR);\n"
"  if (dist > PUDDLE_THRESH - (0.75 * delta) &&\n"
"      dist < PUDDLE_THRESH - (0.75 * delta) + 0.01) {\n"
"    FragColor = vec4(1.0);\n"
"  } else {\n"
"    FragColor = vec4(0.0117, 0.6156, 0.9882, 1.0);\n"
"  }\n"
"}\n"
""
"float noise2(vec2 coords) {\n"
"    return fract(sin(dot(coords, vec2(O, P))) * L);\n"
"}\n"
;
